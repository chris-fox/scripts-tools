...
class ToolValidator(object):
    ...
    def updateParameters(self):
        ...
        if not self.params[3].value:
            ...
            for key in content:
                content[key] = [item for item in content[key] if item.type.lower() == 'web mapping application' or item.type.lower() == 'operations view']
        ...
        if not self.params[1].hasBeenValidated and self.params[1].value is not None and self.params[0].value is not None:
            content = _solution_helpers.get_validation_json()
            folder = self.params[0].valueAsText
            application = next((item for item in content[folder] if item.title == self.params[1].valueAsText), None)
            self.params[2].value = application.url
        return
