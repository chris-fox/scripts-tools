    ...
    def updateParameters(self):
        ...
        if not self.params[0].hasBeenValidated and self.params[0].value is not None:
            ...
            if folder == validation_dict['username']:
                folder = None
            items = [item for item in target.users.me.items(folder) if item.type.lower() == "web mapping application" or item.type.lower() == "operation view"] 
        ...
        if not self.params[1].hasBeenValidated and self.params[1].value is not None and self.params[0].value is not None:
            ...
            application = next((item for item in items if item.title == self.params[1].valueAsText), None)
            if application is None:
                return
            url = application.url
            if application.type.lower() == "operation view":
                url = "{0}apps/dashboard/index.html#/{1}".format(arcpy.GetActivePortalURL(), application.id)
